...
def get_missing_days(save_dir):
    ...
    return missing_dict
def find_weird_timestamps(save=True):
    #paths to raw wavs
    raw_audio_root_1 = '/Volumes/LaCie_barn/mouse_barn_audiomoth/data/audio/raw'
    raw_audio_root_2 = '/Volumes/LaCie_barn2/mouse_barn_audiomoth/data/audio/raw'

    #list of audiomoths
    moths = os.listdir(raw_audio_root_1)

    #deployments in each hard drive
    deployments_1 = np.concatenate([glob.glob(os.path.join(raw_audio_root_1, moth,'*')) for moth in moths]).tolist()
    deployments_2 = np.concatenate([glob.glob(os.path.join(raw_audio_root_2, moth,'*')) for moth in moths]).tolist()

    #boxes recorded for each deployment
    boxes_recorded = load_json('/Volumes/LaCie_barn/mouse_barn_audiomoth/parameters/boxes_recorded.json')

    #initialize list
    strange_wavs = []

    for deployment in deployments_1:

        #get the audiomoth
        moth = deployment.split('/')[-2]

        #get all the wav file names (these are the timestamps)
        raw_wavs = [i for i in glob.glob(os.path.join(deployment, '*.wav')) if not i.startswith('.')]

        #remove the first and last timestamp, which should not end in 00
        raw_wavs = sort_nicely(raw_wavs)[1:-1]

        #find the remaining time stamps that do not end in 00 (this is weird)
        weirds = [i for i in raw_wavs if not i.endswith('00.wav') and not i.startswith('.')]                      

        #append to dictionary
        deployment_abrv = deployment.split('/')[-1].split('_')[0]
        if deployment_abrv not in ["20220623-20220624", "20220621-20220623", "20220623-20220624", "20220624-20220626"]:
            strange_wavs.extend(weirds)

    #add the other deployments
    for deployment in deployments_2:
    ...
    if save:
        file_path = '/Volumes/LaCie_barn/mouse_barn_audiomoth/parameters/non_00_seconds.json'
        with open(file_path, "w") as file:
            json.dump(strange_wavs, file)
       
    return strange_wavs
def check_weird_timestamp_lengths():
    #check that all timestamps that are unusual have a length of 54 seconds
    #returns nothing if so, throughs an error if not
    from scipy.io import wavfile
    weird_stamps = load_json('/Volumes/LaCie_barn/mouse_barn_audiomoth/parameters/non_00_seconds.json')
    
    for weird_stamp in weird_stamps:

        fs, wav = wavfile.read(weird_stamp)
        length_in_s = len(wav)/fs
        print(weird_stamp.split('/')[-1],':',length_in_s)
        assert length_in_s == 54
#def find_weird_wavs():
    
    # find deployments in which more than 2 recordings have a file size other that 21.1 MB
def strptime_ms(val):
...
def get_recording_windows(root):
	"""
	...
	# check that you have access to all the audio data (ie, both external hard-drives)
	assert os.path.exists('/Volumes/LaCie_barn'), "Lacie_barn is not connected"
	assert os.path.exists('/Volumes/LaCie_barn2'), "Lacie_barn2 is not connected"
	assert os.path.exists('/Volumes/LaCie_barn3'), "Lacie_barn3 is not connected"

	# boxes
	boxes = [str(i) for i in np.arange(2,41,2)]

	#deployments
	deployments = get_deployments(root = root)

	# initialize the dictionary
	recording_window_dict = {}

	for deployment in deployments:

		# initialize nested dict
		recording_window_dict[deployment] = {} 

		# find which hard drive the raw audio is on
		locations = load_json(os.path.join(root, 'parameters', 'recording_storage_locations.json'))
		raw_audio_root = locations[deployment]

		# loop through the moths to build the inner dictionary
		for box in boxes:

			# get the deployments where this box was recorded by any audiomoth
			deployment_box = glob.glob(os.path.join(raw_audio_root, "*", deployment+"_box"+box))
			
			if len(deployment_box) == 0: # ignore the boxes that were not recorded this deployment
				continue
			
			assert len(deployment_box) == 1, "There can only be one recording per box per deployment"
			# if the recording failed at any point skip it
			if contains_empty_wavs(os.path.join(deployment_box[0])):
				continue

			else:

				# get the file names
				wavfiles = [i for i in os.listdir(deployment_box[0]) if not i.startswith('.') and i.endswith('.wav')]

				# get the start and stop of the recording
				timestamps = [audiomoth_to_datetime(i.split('.')[0]) for i in wavfiles]

			# fill in the dictionary
			recording_window_dict[deployment][box] = (str(min(timestamps)), str(max(timestamps)))

	return recording_window_dict
